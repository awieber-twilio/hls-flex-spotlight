...
``` -->
3. If you do not already have a GitHub account, create one. Log in to your GitHub account in your local terminal. You may need to create a [personal access token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token).
...
npm run export
```
8. Next copy the assets created by the previous command into a folder that matches the structure for Twilio Services by running 
```
cp -r out/* ../assets
```
...
```
9. Release the plugin by copying and pasting the output from the previous command. It should look something like this: `twilio flex:plugins:release --plugin hls-emr@0.0.2 --name "Autogenerated Release xxxxxxxx" --description "The description of this Flex Plugin Configuration."`
